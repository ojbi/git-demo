...
<div class="JobtableLayout">
  <div class="jobsma">
  <img src="../images/jobsm.png" width="100px" height="100px"><h3>  Closed Jobs</h3>
</div>
...
//count all jobs
    ...
    //count close jobs
     ...
     $total = mysql_query("Select * from jobs_opening,applicant_info where jobs_opening.posid='applicant_info.posid'")or die (mysql_error());
     $totalapp=mysql_num_rows($sql);

?>
  <div class="jobstat">
  <h5> Status: <a href="jobs.php">View All (<?php echo $all;?>) </a> | <a href="open.php"> Open ( <?php echo $op; ?>) </a> | <a href=""> Close (<?php echo $close;?>) </a></h5>
</div>
...
<br>
 ...
 <?php
        //retriev all users
        $sql=mysql_query("Select * from jobs_opening where status='close'")or die (mysql_error());
          // if no users yet
          if (!$sql) 
          {
              ...
              <input type = 'hidden' name = 'posid' value = <?php echo '$posid'; ?>>
                <tr class='record' onmouseover='this.style.backgroundColor=#0080FF;'  onmouseout='this.style.backgroundColor=#d4e3e5;'>
                  ...
                  <td style='overflow:hidden; width:220px;' align='center'> <?php echo date('F d, Y', strtotime($cdate)); ?></td>
                   <td style='overflow:hidden; width:220px;' align='center'> <?php echo $totalapp; ?></td>
