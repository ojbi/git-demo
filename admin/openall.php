...
?>
  <div class="jobstat">
  <h5> Status: <a class="<?=($st=='all') ? 'active':'' ?>" href="jobs.php?st=all">View All (<?php echo $all;?>) </a> | <a class="<?=($st=='open') ? 'active':'' ?>" href="jobs.php?st=open"> Open ( <?php echo $op; ?>) </a> | <a class="<?=($st=='close') ? 'active':'' ?>" href="jobs.php?st=close"> Close (<?php echo $close;?>) </a></h5>
</div>
...
<br>
 ...
 $sql=mysql_query("Select * from jobs_opening")or die (mysql_error());
          // if no users yet
          if (!$sql) 
          {
...
<table class = 'hovertable'  style='table-layout:fixed' align = 'center'>   
              <input type = 'hidden' name = 'posid' value = <?php echo '$posid'; ?>>
                <tr class='record' onmouseover='this.style.backgroundColor=#0080FF;'  onmouseout='this.style.backgroundColor=#d4e3e5;'>
                  ...
                  ?>
                   <td style='overflow:hidden; width:220px;' align='center'> <?php echo $tot; ?></td>
 ...
 {
        //retriev all users
        $sql=mysql_query("Select * from jobs_opening where status = '$st'")or die (mysql_error());
          // if no users yet
          if (!$sql) 
          {
            echo "<p>Couldn't get record!";
